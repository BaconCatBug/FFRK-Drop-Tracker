...
depricated_comma_seperated_export = 0
...
def getRelics(data, filename):
    ...
    for elem in data:
        ...
        rarity = elem['rarity']
        evolution_num = elem['evolution_num']
        ...
        if modified_id_export == 1 and elem['rarity'] == 101:
            if (evolution_num == 3):
                relic_id = relic_id*10 + 3
                rarity += 3
            elif (evolution_num == 2):
                relic_id = relic_id*10 + 2
                rarity += 2
            elif (evolution_num == 1):
                relic_id = relic_id*10 + 1
                rarity += 1
            else:
                relic_id = relic_id*10
...
def getSBs(data, filename, n):
    teir = {0: '???', 1: 'Default', 2: 'Shared', 3: 'Unique', 4: 'Super', 5: 'Burst', 6: 'Overstrike', 7: 'Ultra',
            8: 'Chain', 9: 'Arcane Overstrike', 10: 'Glint', 11: 'Awakening', 12: 'Syncro', 13: 'Limit Break Overstrike', 14: 'Limit Break Glint', 15: 'Arcane Dyad', 16: '???', 17: '???', 18: '???', 19: '???', 20: '???', 21: '???', 22: '???'}
    elems = []
    ...
    if depricated_comma_seperated_export:
        ...
        with open('{}.txt'.format(filename), 'w', encoding="utf-8") as f:
            ...
            for elem in elems:
                f.write('\n{}\t{}\t{}\t{}\t{}\t{}\t{}'.format(elem[0], elem[1], elem[2], elem[3], elem[4], elem[5],
                                                                      elem[6]))
        if path.exists('5-FFRK-Magicite2.txt') and path.exists('5-FFRK-Magicite1.txt'):
            data = data2 = ""
            with open('5-FFRK-Magicite1.txt') as fp: 
                data = fp.read()
            with open('5-FFRK-Magicite2.txt') as fp: 
                data2 = fp.read()
            data += '\n'
            data += data2
            with open ('5-FFRK-Magicite.txt', 'w') as fp: 
                fp.write(data)
            remove('5-FFRK-Magicite1.txt')
            remove('5-FFRK-Magicite2.txt')
...
def getCharacters(data, filename):
    elems = []
...
def exportRawJson(data, filename, part):
    with open(part+('{}.json'.format(filename)), 'w', encoding="utf-8") as json_file:
        dump(data, json_file)
